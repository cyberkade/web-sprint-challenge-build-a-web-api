...
function validateNewAction(req, res, next) {
    const { notes, description, project_id } = req.body
      if(notes && notes.trim() && description && description.length <= 128 && project_id){
...
function validateUpdatedAction(req, res, next) {
    const { notes, description, completed, project_id } = req.body
      if(notes && notes.trim() && description && description.length <= 128 && project_id && typeof completed === 'boolean'){
        req.newAction = req.body
        next()
      } else {
        next({status: 400, message: "missing required fields"})
